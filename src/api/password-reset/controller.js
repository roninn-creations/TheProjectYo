...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your TheProjectYo account.<br>
        ...
        &mdash; TheProjectYo Team
      `
      return sendMail({ toEmail: email, subject: 'TheProjectYo - Password Reset', content })
    })
